...
import schnetpack
import schnet_ev
from schnet_ev.base import Build
from schnetpack import nn as L, Properties
from schnet_ev.blocks import MLP
__all__ = [
    "Atomwise",
    "AtowiseH",
    "ElementalAtomwise",
...
class AtomwiseH(nn.Module):
    ...
    def __init__(
        ...
        if aggregation_mode == "sum":
            ...
            self.atom_pool = Build(axis=1, mean=True)
        elif aggregation_mode =="build":
            self.atom_pool = Build(axis=1,mean=False)
        elif aggregation_mode =="build_atomic":
            self.atom_pool = Build_Atomic(axis=1,mean=False)
        else:
            raise AtomwiseError(
    ...
    def forward(self, inputs):
        ...
        if self.atomref is not None:
            ...
            yi = yi + y0
        #change here to make adaptive vector with length 1/2  * atomic_numbers
        y,n_y = self.atom_pool(yi,atomic_numbers, atom_mask)
...
class Atomwise(nn.Module):
    ...
    def __init__(
        ...
        if aggregation_mode == "sum":
            ...
            self.atom_pool = Build(axis=1,mean=False)
        elif aggregation_mode == "do_nothing":
            self.atom_pool = do_nothing(axis=1,mean=False)
        else:
            raise AtomwiseError(
                "{} is not a valid aggregation " "mode!".format(aggregation_mode)
            )
