...
class do_nothing(nn.Module):
    ...
    def forward(self, input,a_z, mask=None):
...
class Build(nn.Module):
    ...
    def forward(self, input,a_z, mask=None):
        ...
        y_shape = torch.sum(a_z,1)
        y_shape2= torch.max(y_shape)+2
        if int(y_shape2*y_shape2/2+y_shape2) > input.shape[2]:
            y = torch.sum(input, self.axis)
            y2 = input.shape[2]
        else:
            y = torch.sum(input, self.axis)#[:,:int(y_shape2*y_shape2/2+y_shape2)]
            y2 = int(y_shape2/2)
        if self.average:
...
class Build_Atomic(nn.Module):
    ...
    def __init__(self, axis, mean=False, keepdim=True):
        super(Build_Atomic, self).__init__()
        self.average = mean
        self.axis = axis
        self.keepdim = keepdim
    ...
    def forward(self, input,a_z, mask=None):
        ...
        # compute sum of input along axis
        y_shape = torch.sum(a_z,1)
        y_shape2= torch.max(y_shape)
        factor = input.shape[2]/y_shape[0]
        #y =torch.zeros((input.shape[0],input.shape[2]),device=input.device)
        y = torch.zeros((input.shape[0],y_shape2,y_shape2),device=input.device)
        #y = torch.sum(input,self.axis)
        for i in range(input.shape[0]):
            index = 0
            for j in range(input.shape[1]):
                for k in range(a_z[i,j]):
                    y[i,index+k,:] = input[i,j,(y_shape2*k):(y_shape2+k*y_shape2)]
                    #y[i,index:(index+a_z[i,j]*factor)] = a[:a_z[i,j]*factor]
                index += a_z[i,j]
        return y
