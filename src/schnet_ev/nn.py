...
def diagonal_phaseloss(target, predicted, n_orb,device,single):
    ...
    nmstates=n_orb['n_occ']+3*n_orb['n_unocc']
    #torch tensor
    ...
    all_energies[:,:n_orb['n_occ']+n_orb['n_unocc']] = energy_predicted[:,:]
    all_energies[:,n_orb['n_occ']+n_orb['n_unocc']:n_orb['n_occ']+n_orb['n_unocc']*2] = energy_predicted[:,n_orb['n_occ']:]
    all_energies[:,n_orb['n_occ']+n_orb['n_unocc']*2:n_orb['n_occ']+n_orb['n_unocc']*3] = energy_predicted[:,n_orb['n_occ']:]
...
def min_loss_single(target, predicted, combined_phaseless_loss, n_orb, props_phase, phase_vector_nacs = None, dipole=False):
    ...
    #number of couplings
    #number of possibilities of phase switches (n_orb are the number of S + T states)
...
def min_loss(target, predicted, combined_phaseless_loss, n_orb, props_phase, phase_vector_nacs = None, dipole=False):
    ...
    if combined_phaseless_loss == True:
      ...
      for index_batch_sample in range(batch_size):
          ...
          for index_phase_vector in range(n_phases):
              ...
              coupling_iterator=0
              for i_singlet in range(n_orb['n_occ']):
                  for j_singlet in range(i_singlet+1,n_orb['n_occ']):
              ...
              for i_triplet in range(n_orb['n_occ'],n_orb['n_occ']+n_orb['n_unocc']):
                  for j_triplet in range(i_triplet+1,n_orb['n_occ']+n_orb['n_unocc']):
...
def combined_loss(phase_vector_nacs, n_orb, n_socs, all_states, device):
    phase_vector_nacs_1 = torch.Tensor(phase_vector_nacs.size()[0],all_states).to(device)
    phase_vector_nacs_1[:,:n_orb['n_occ']+n_orb['n_unocc']] = phase_vector_nacs
    #append the triplet part
    phase_vector_nacs_1[:,n_orb['n_occ']+n_orb['n_unocc']:n_orb['n_occ']+n_orb['n_unocc']*2] = phase_vector_nacs[:,n_orb['n_occ']:]
    phase_vector_nacs_1[:,n_orb['n_occ']+n_orb['n_unocc']*2:n_orb['n_occ']+n_orb['n_unocc']*3] = phase_vector_nacs[:,n_orb['n_occ']:]

    phase_vector_nacs_2 = torch.Tensor(phase_vector_nacs.size()[0],all_states).to(device)
    phase_vector_nacs_2[:,:n_orb['n_occ']] = phase_vector_nacs_1[:,:n_orb['n_occ']]
    phase_vector_nacs_2[:,n_orb['n_occ']:] = phase_vector_nacs_1[:,n_orb['n_occ']:] * -1
...
class DipoleMask(nn.Module):
    ...
    def __init__(self, n_orb, n_occ=1, n_unocc=None, expansion=3, fit=False, true_inverse=True):
        super(DipoleMask, self).__init__()
        self.n_occ=n_occ
        self.n_unocc=n_unocc
        self.nmstates=n_unocc*3+n_occ
    def forward(self, dipole_moments):
        ...
        dipole_triu_mask = torch.triu(torch.ones(self.n_occ+3*self.n_unocc,self.n_occ+3*self.n_unocc), 0).long() == 1
        dipole_triu_mask[:self.n_occ,self.n_occ:]=0

        if self.n_unocc is not None:
          dipole_triu_mask[:self.n_occ+self.n_unocc,self.n_occ+self.n_unocc:]=0
          dipole_triu_mask[:self.n_occ+2*self.n_unocc,self.n_occ+self.n_unocc*2:]=0
...
class ApproximateInverse(nn.Module):
    ...
    def __init__(self, n_orb, n_unocc=None, expansion=3, fit=False, true_inverse=True):
        super(ApproximateInverse, self).__init__()

        # Construct the mask
        triu_mask = torch.triu(torch.ones(n_orb, n_orb), 1).long() == 1
        if n_unocc is not None:
            triu_mask[:n_unocc, n_unocc:] = 0
